...
import { getPreviousMonthNames, disableFutureDates } from 'helpers/utils';
...
const AdminComparison = () => {
  ...
  const onClickExportAsExcel = () => {
    ...
    saveAs(blob, 'Admin-Profit-Loss-Report.xlsx');
  };

  const getSiteSalaryDetails = (month, year, token) => {
    const departmentServices = 'Admin';
    invoiceApi
      .getAdminProfitAndLoss(month + 1, year, departmentServices, token)
      .then((response) => {
      ...
      <Card>
        <Card.Header className="bg-shape modal-shape-header px-20 position-relative">
          <div className="position-relative light">
            <h5 className="mb-0 text-white"> Admin Services</h5>
          </div>
        </Card.Header>
        <br />
                      ...
                      return labels.map((label, labelIndex) => {
                        ...
                        switch (label) {
                          ...
                          case 'EXPENSES':
                            ...
                            break;
                          case 'BONUS':
                            value = formattedAmount(item.bonus);
                            break;
                          case 'DEPARTMENT OH SALARY':
                            value = formattedAmount(item.departmentOHSalary);
                            break;
                          case 'DEPARTMENT OH EXPENSES':
                            value = formattedAmount(item.departmentOHExpenses);
                            break;
                          case 'COMPANY OH':
                          ...
                          case 'GROSS PROFIT': {
                            const netResult =
                              ...
                              item.expenses -
                              item.bonus -
                              item.departmentOHSalary -
                              item.departmentOHExpenses -
                              item.companyOH -
                              item.departmentOH -
                              item.esi -
                              item.pf;
