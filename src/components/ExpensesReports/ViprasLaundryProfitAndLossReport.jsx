...
const ViprasLaundryProfitAndLossReport = () => {
  const { user } = useAuth();
  ...
  const getSiteSalaryDetails = (month, year) => {
    invoiceApi
      .getServicesProfitAndLoss(month + 1, year, 'Laundry Service')
      .then((response) => {
        ...
        <Card.Header className="bg-shape modal-shape-header px-20 position-relative">
          <div className="position-relative light">
            <h5 className="mb-0 text-white">
              {' '}
              Laundry Profit And Loss Report
            </h5>
          </div>
        </Card.Header>
        <br />
...
export default ViprasLaundryProfitAndLossReport;
