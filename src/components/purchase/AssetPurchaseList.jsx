      ...
      return (
        <CardDropdown iconClassName="fs--1" drop="start">
          <div className="py-2">
            <Dropdown.Item
              as={Link}
              to={`/assetpurchasesingleview/${get(
                rowData,
                'row.values.id',
                '',
              )}`}
            ...
            <Dropdown.Item
              as={Link}
              to={`/assetpurchase/${get(rowData, 'row.values.id', '')}`}
...
function AssetPurchaseList() {
  ...
  const { isLoading, error, data, isSuccess } = useQuery(
    ...
    () =>
      purchaseAPI.fetchpurchaseAssetData(
        queryPageIndex,
        queryPageSize,
        queryPageFilter,
        todayYears,
  ...
  const onClickExportAsExcel = () => {
    ...
    const filename = `Asset_Purchase_${formattedDate}.xlsx`;
...
export default AssetPurchaseList;
