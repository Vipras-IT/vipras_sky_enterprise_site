...
const SubcontractorRepordsTemp = () => {
  const { user } = useAuth();
  const todayDate = new Date(2025, 5, 1);
  ...
  const showUpdateEmployee = async (month, year) => {
    const response = await subContractorApi.getSubContractorReportTemp(
      month,
      year
    );
...
export default SubcontractorRepordsTemp;
