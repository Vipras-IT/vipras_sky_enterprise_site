...
const BestSellingTableRows = ({ product }) => {
  const { employeeId, employeeName, debit, credit, monthlyBalance, openingBalance } = product;
  ...
  return (
    <tr className="border-top border-200 ">
      ...
      <td className="align-middle text-center fw-semi-bold ">
        {formattedAmount(monthlyBalance)}
      </td>
    </tr>
  );
};

const SubcontractorMonthlyReports = () => {
  const { user } = useAuth();
  ...
  const showUpdateEmployee = async (month, year) => {
    ...
    if (errorMessage) {
      ...
      const totalCashInHand = res.reduce(
        (accumulator, item) => accumulator + (item.monthlyBalance || 0),
        0,
      );
      ...
      finalTotal.push({
        ...
        debit: totalDebit,
        credit: totalCredit,
        monthlyBalance: totalCashInHand,
      });
    ...
    <Card className="overflow-hidden">
      <Card.Header className="bg-shape modal-shape-header px-20 position-relative">
        <div className="position-relative light">
          <h5 className="mb-0 text-white">Sub Contractor - Monthly Summary Reports</h5>
        </div>
      </Card.Header>
...
export default SubcontractorMonthlyReports;
