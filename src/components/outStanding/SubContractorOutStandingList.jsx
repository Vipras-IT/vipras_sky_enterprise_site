...
const SubContractorOutStandingList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await ImprestApi.getSubSite('SUB_CONTRACTOR');
  ...
  const getSiteSalaryDetails = (siteId, month, year) => {
    ...
    ImprestApi.fetchSubContractorInvoiceOutStanding(
      siteId,
      monthLabel,
      year,
      searchType,
      ...
      .then((response) => {
        ...
        const totalDebitAmount = responseData.reduce(
          (accumulator, item) => accumulator + (item.debitAmount || 0),
          0,
        );
        const totalCreditAmount = responseData.reduce(
          (accumulator, item) => accumulator + (item.creditAmount || 0),
          0,
        );
        const totalTdsAmount = responseData.reduce(
          (accumulator, item) => accumulator + (item.tdsAmount || 0),
          0,
        );
        const finalCreditResult = totalCreditAmount + totalTdsAmount;
        const finalDebitResult =
          totalDebitAmount + get(responseData[0], 'openingBalance', 0);
        ...
        <Card.Header className="bg-shape modal-shape-header px-20 position-relative">
          <div className="position-relative light">
            <h5 className="mb-0 text-white">
              Sub Contractor Out Standing Report
            </h5>
          </div>
        </Card.Header>
        <br />
...
export default SubContractorOutStandingList;
