...
import ManualAttendanceReportHeader from './ManualAttendanceReportHeader';
...
const columns = [
  ...
  },
  {
    ...
    Header: 'Created On',
    Cell: (rowData) => {
      const saleDate = new Date(get(rowData, 'row.values.createdOn', null));
      const month = saleDate.getMonth() + 1;
      const monthValue = month < 10 ? `0${month}` : `${month}`;
      const day = saleDate.getDate();
      const dayValue = day < 10 ? `0${day}` : `${day}`;
  ...
  },
  {
    ...
    Header: 'Updated On',
    Cell: (rowData) => {
      const saleDate = new Date(get(rowData, 'row.values.updatedOn', null));
      const month = saleDate.getMonth() + 1;
      const monthValue = month < 10 ? `0${month}` : `${month}`;
      const day = saleDate.getDate();
      const dayValue = day < 10 ? `0${day}` : `${day}`;
...
function ManualAttendanceReport() {
  ...
  // );
  // const getSiteDetails = (sitId) => {
  //   getSiteAPI.request(sitId);
  // };
  ...
  const { isLoading, error, data, isSuccess } = useQuery(
    ...
    () =>
      ...
      manualAttendanceAPI.fetchManualattendanceData(
        queryPageIndex,
        queryPageSize,
        queryPageFilter,
        // dateFilter,
