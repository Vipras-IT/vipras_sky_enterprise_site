...
const columns = [
  ...
  },
  {
    ...
    Header: 'Time',
    Cell: ({ row }) => {
      const value = row.values.createdOn;
  ...
  },
  {
    ...
    Header: 'Image',
    Cell: (rowData) => {
      const value = get(rowData, 'row.values.imageUrl', []);
      return value && value.length > 0 ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <img src={value} alt="Image" width="50" height="50" />
...
function FaceList() {
  ...
  const { isLoading, error, data, isSuccess } = useQuery(
    ...
    () =>
      resume.getFaceAttendance(queryPageIndex, queryPageSize, queryPageFilter),
    {
          ...
          <Card>
            <Card.Header className="bg-shape modal-shape-header px-4 position-relative">
              <div className="position-relative z-index-1 light">
                <h5 className="mb-0 text-white">Face List</h5>
              </div>
            </Card.Header>
