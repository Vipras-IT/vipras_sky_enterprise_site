...
// import SalesManagement from './SalesHeader';
...
import ImprestHolderTransactionHeader from 'components/imprestHolderTransaction/ImprestHolderTransactionHeader';
import DmrReportHeader from './DmrReportHeader';
...
const columns = [
  ...
  },
  {
    ...
    Header: 'Documents',
    Cell: (rowData) => (
      <Link target="_blank" to={get(rowData, 'row.values.url', null)}>
        Your Document
      </Link>
    ),
  },
  ...
  //             <Dropdown.Item
  //               as={Link}
  //               to={`/sale/${get(rowData, 'row.values.id', '')}`}
...
function DmrReport() {
  ...
  const { isLoading, error, data, isSuccess } = useQuery(
    ...
    () =>
      documentAPI.getDmrDocumentByFilter(
        queryPageIndex,
        queryPageSize,
        queryPageFilter,
        token,
  ...
  const exportColumn = [
    ...
    {
      accessor: 'deductionDuration',
      Header: 'Deduction Duration',
    },
  ];
  ...
  const onClickExportAsExcel = () => {
    ...
    const filename = `Sales_Retain_${formattedDate}.xlsx`;
          ...
          <Card>
            <Card.Header className="bg-shape modal-shape-header px-4 position-relative">
              <div className="position-relative z-index-1 light">
                <h5 className="mb-0 text-white"> DMR Report</h5>
              </div>
            </Card.Header>
            <Card.Header>
              <DmrReportHeader
