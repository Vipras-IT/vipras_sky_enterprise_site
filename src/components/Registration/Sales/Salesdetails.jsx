...
import salesManagementAPI from 'api/deduction';
...
const columns = [
  ...
  },
  {
    ...
    Cell: (rowData) => {
      return <>{get(rowData, 'row.values.deductionDuration', []).join('-')}</>;
    },
  },
  {
    accessor: 'id',
...
function ListSales() {
  ...
  const { isLoading, error, data, isSuccess } = useQuery(
    ...
    () =>
      salesManagementAPI.fetchSalesData(
        queryPageIndex,
        queryPageSize,
        queryPageFilter,
      ),
          ...
          <Card>
            <Card.Header className="bg-shape modal-shape-header px-4 position-relative">
              <div className="position-relative z-index-1 light">
                <h5 className="mb-0 text-white">All Sales Report</h5>
              </div>
            </Card.Header>
            <Card.Header>
              <SalesManagement
            ...
            <Card.Footer>
              <div className="pagination">
                ...
                <div>
                  {' '}
                  <b>TOTAL AMOUNT : {formattedAmount(granTotalAmount)}</b>
                </div>
                <div>
                  <Form.Select
                    size="sm"
                  ...
                  >
                    {[100, 200, 300, 400, 500, 'All'].map((pageSize) => (
                      <option
                        key={pageSize}
                        value={
                          pageSize === 'All' ? data?.count: pageSize
                        }
                      >
                        Show {pageSize}
                      </option>
