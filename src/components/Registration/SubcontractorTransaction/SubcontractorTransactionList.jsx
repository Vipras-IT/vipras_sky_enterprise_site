...
import SubcontractorFilterManagement from './SubcontractorFilter';
...
import { formattedAmount,calculateGrandTotalAmount } from 'helpers/utils';
...
const columns = [
  {
    ...
    Cell: (rowData) => {
      if (get(rowData, 'row.values.date', null) === 'TOTAL') {
  ...
  },
  {
    ...
    Header: 'Total Amount',
    Cell: (rowData) => (
      <>{formattedAmount(rowData.row.values.totalAmount ?? '')}</>
    ),
  },
  {
    ...
    Cell: (rowData) => (
      <>{formattedAmount(rowData.row.values.receivedAmount ?? '')}</>
    ),
  },
  {
    ...
    headerProps: { className: 'pe-7' },
    Cell: (rowData) => {
      if (get(rowData, 'row.values.receivedDate', null) === 'receivedDate') {
        return <span> </span>;
      }
      const saleDate = new Date(get(rowData, 'row.values.receivedDate', null));
      const month = saleDate.getMonth() + 1;
      const monthValue = month < 10 ? `0${month}` : `${month}`;
      const day = saleDate.getDate();
      const dayValue = day < 10 ? `0${day}` : `${day}`;
  ...
  },
  {
    ...
    Header: 'TDS Amount',
    Cell: (rowData) => (
      <>{formattedAmount(rowData.row.values.tdsAmount ?? '')}</>
    ),
  },
  {
    accessor: 'otherDeduction',
    Header: 'other Deduction',
    Cell: (rowData) => (
      <>{formattedAmount(rowData.row.values.otherDeduction ?? '')}</>
    ),
  },
  {
    accessor: 'otherDeductionRemarks',
    Header: 'other Deduction Remarks',
  },
  {
    accessor: 'handOverToMD',
    Header: 'Cash To Bank',
    Cell: (rowData) => (
      <>{formattedAmount(rowData.row.values.handOverToMD ?? '')}</>
    ),
  },
  {
    accessor: 'imprestByMyself',
    Header: 'Imprest By Myself',
    Cell: (rowData) => (
      <>{formattedAmount(rowData.row.values.imprestByMyself ?? '')}</>
    ),
  },
  {
    accessor: 'id',
      ...
      return (
        <CardDropdown iconClassName="fs--1" drop="start">
          <div className="py-2">
            {/* <Dropdown.Item
              as={Link}
              to={`/salesingleview/${get(rowData, 'row.values.id', '')}`}
            ...
            <Dropdown.Item
              as={Link}
              to={`/subcontractorTransaction/${get(
                rowData,
                'row.values.id',
                '',
              )}`}
...
function SubContractorTransactionList() {
  ...
  const { isLoading, error, data, isSuccess } = useQuery(
    ...
    () =>
      subcontractorAPI.fetchSubContractorTransactionData(
        queryPageIndex,
        queryPageSize,
        queryPageFilter,
      ),
  ...
  const finalTotalAmount = calculateGrandTotalAmount(data?.results, 'totalAmount');
  const finalReceivedAmount = calculateGrandTotalAmount(data?.results, 'receivedAmount');
  ...
  const onClickExportAsExcel = () => {
    ...
    const filename = `SubContractor_Transactions_Report_${formattedDate}.xlsx`;
          ...
          <Card>
            <Card.Header className="bg-shape modal-shape-header px-4 position-relative">
              <div className="position-relative z-index-1 light">
                <h5 className="mb-0 text-white">
                  All SubContractor Transactions Report
                </h5>
              </div>
            </Card.Header>
            <Card.Header>
              <SubcontractorFilterManagement
            ...
            <Card.Footer>
              <div className="pagination">
                ...
                <div>
                  {' '}
                  <b>TOTAL AMOUNT : {formattedAmount(finalTotalAmount)}</b>
                </div>
                <div>
                  {' '}
                  <b>TOTAL RECEIVED : {formattedAmount(finalReceivedAmount)}</b>
                </div>
                <div>
                  <Form.Select
                    size="sm"
...
export default SubContractorTransactionList;
